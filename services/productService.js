...
async function getProductsByCategory(category) {
  ...
  // 回傳格式：篩選後的產品陣列
  return (await fetchProducts()).filter(item => item.category === category)
...
async function getProductById(productId) {
  ...
  // 若找不到，回傳 null
  return (await fetchProducts()).find(item => item.id === productId) || null
}
...
async function getCategories() {
  ...
  return getAllCategories((await fetchProducts()))
}
...
function displayProducts(products) {
  ...
  //    售價：NT$ 800 (8折)
  // ----------------------------------------
  const result = products.forEach(function(item, index){
    return(`
      產品列表：
      ----------------------------------------
      ${index+1}. ${item.title}
