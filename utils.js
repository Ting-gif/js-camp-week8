...
function getDiscountRate(product) {
  // 請實作此函式
  const result = product.price / product.origin_price * 10
  return `${Math.round(result)}折`
}
...
function validateOrderUser(data) {
  ...
  if (!data.email || !data.email.includes('@')) errors.push('Email 格式錯誤')
...
function formatCurrency(amount) {
  // 請實作此函式
  const result = Number(amount).toLocaleString('zh-TW')
  return `NT$ ${result}`
}
